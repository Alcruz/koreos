 ...
 * one of these: sizeof(page_t) == PAGE_SIZE, so the type documents "exactly one
 * frame" and pointer arithmetic steps a whole frame at a time. It points at the
 * frame's bytes, not a descriptor — there is no per-frame metadata. */
typedef struct page {
